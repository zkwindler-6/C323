 ...
 * [read-only]
 * 
 * This class represents a single LineSegment, composed from two
 * endpoints.  This class is used by the GUI as the user is drawing
 * the line. When that's happening, the LeftEndpoint is fixed, but the
 * RightEndpoint is still changing. Methods are provided to draw the
 * line segment using a graphics context, with or without details and
 * highlighting.  Methods are provided to test for intersection with
 * another line segment or a point.
 * 
 * Importantly, the lessThan() predicate allows us to determine the
 * relative order of this line segment and some other line segment
 * according to the y-intercepts with the sweep line. This is the main
 * computation performed by the relation on which Sweeper maintains
 * its tree.
 */
...
public class LineSegment {
  ...
  public boolean intersects(LineSegment that) {
    return new Line2D.Double(p1, p2).
        intersectsLine(new Line2D.Double(that.p1, that.p2));
  }
    ...
    if (p2 != null) {
      ...
      if (showDetails)
        if (!fixed) {
          ...
          g2d.drawString(left.toString(),
              Math.max((int) left.getX() - LDELTA, 0), (int) left.getY());
          g2d.setColor(RIGHT_COLOR);
          g2d.drawString(right.toString(),
              (int) right.getX() + RDELTA, (int) right.getY());
        }
        else {
          final int RADIUS = 5;
          int diameter = 2 * RADIUS;
          g2d.setColor(LEFT_COLOR);
          g2d.fillOval((int) left.getX() - RADIUS,
              (int) left.getY() - RADIUS, diameter, diameter);
          g2d.setColor(RIGHT_COLOR);
          g2d.fillOval((int) right.getX() - RADIUS,
              (int) right.getY() - RADIUS, diameter, diameter);
          g2d.setColor(Color.BLACK);
          g2d.drawOval((int) left.getX() - RADIUS,
              (int) left.getY() - RADIUS, diameter, diameter);
          g2d.drawOval((int) right.getX() - RADIUS,
              (int) right.getY() - RADIUS, diameter, diameter);
        }
    }
  }
 ...
 * window to the bottom border, and it's completely vertical. It is drawn as
 * a dashed gray line.
 */
...
class SweepLine extends LineSegment {
  ...
  public SweepLine(int x) {
    super(new Point2D.Double(x, 0),
        new Point2D.Double(x, Constants.SURFACE_HEIGHT));
