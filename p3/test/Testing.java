...
public class Testing {
    ...
    /**
     * Start with this test. Make appropriate changes to DoublyLinkedList so
     * that it supports the search() operation. Be sure to look at the Location
     * interface.
     */
    ...
    public void DLLsearch() {
        List<String> pets = new DoublyLinkedList<>();
        assertNull(pets.search("cat"));
        pets.add("cat");
        pets.add("dog");
        pets.add("pig");
        pets.add("cow");
        assertNull(pets.search("bat"));
        Location<String> loc = pets.search("pig");
        assertEquals("pig", loc.get());
        assertEquals("dog", loc.getBefore().get());
        assertEquals("cow", loc.getAfter().get());
        assertEquals("pig", loc.getAfter().getBefore().get());
        assertEquals("cat", loc.getBefore().getBefore().get());
        assertNull(loc.getBefore().getBefore().getBefore());
        assertNull(loc.getAfter().getAfter());
    }

    /**
     * Make the appropriate changes to BinarySearchTree.Node to pass this test.
     * <p>
     * When you're ready to run this test, remove the // from the following line.
     */
    ...
    public void skinnyBST() {
        BinarySearchTree<String> bst =
                new BinarySearchTree<>((x, y) -> x.compareTo(y) < 0);
        String[] a = new String[]{
                "ape", "boa", "cat", "dog", "emu", "fox", "gnu", "hog"
        };
        for (String key : a)
            bst.insert(key);
        ...
        assertEquals("gnu",
                bst.root.right.right.right.right.right.right.data);
        assertEquals("hog",
                bst.root.right.right.right.right.right.right.right.data);
    }
    ...
    public void searchEqualsBST() {
        class Pet {
            ...
            public boolean equals(Object that) {
                return name.toLowerCase().equals(((Pet) that).name.toLowerCase());
            }
        }
        Tree<Pet> bst = new BinarySearchTree<>(
                (p1, p2) -> p1.name.toLowerCase().compareTo(p2.name.toLowerCase()) < 0
        );
        String[] animals = new String[]{
                "cat", "boa", "fox", "emu", "dog", "hog", "asp"
        };
        for (String animal : animals)
            bst.insert(new Pet(animal));
        Location<Pet> loc;
        for (String animal : animals) {
            assertTrue(bst.contains(new Pet(animal)));
            assertTrue(bst.contains(new Pet(animal.toUpperCase())));
            loc = bst.search(new Pet(animal.charAt(0) +
                    animal.substring(1).toUpperCase()));
            assertTrue(loc.get().name.equals(animal));
        }
    }
    ...
    public void reinsertAfterRemoveBST() {
        ...
        assertTrue(bst.isEmpty());
        int[] a = new int[]{4, 8, 0, 2, 6, 10};
        List<Location<Integer>> locs = new DoublyLinkedList<>();
        for (Integer key : a)
    ...
    public void keysDirtyBST() {
        BinarySearchTree<Integer> bst =
                new BinarySearchTree<>((x, y) -> x < y);
        int[] a = new int[]{
                26, 5, 28, 32, 4, 8, 0, 2, 3, 6, 10,
                12, 22, 1, 14, 20, 16, 18, 7, 24, 30
        };
        for (Integer key : a)
            bst.insert(key);
        List<Integer> ks = bst.keys();
    ...
    public void bigBSTtest() {
        ...
        List<Integer> a = new DoublyLinkedList<>();
        for (int i = 0; i < 500; i++)
            a.add(gen.nextInt(i + 1));
        for (Integer x : a) {
            bst.insert(x);
            assertTrue(verifyParentPointers(bst.root));
    ...
    public void beforeBST() {
        ...
        assertTrue(bst.isEmpty());
        int[] a = new int[]{
                12, 4, 18, 5, 11, 8, 15, 9, 17, 20, 3,
                13, 19, 2, 14, 7, 6, 10, 1, 16
        };
        int n = a.length;
        for (Integer key : a)
            bst.insert(key);
    ...
    public void mostlyDirty() {
        ...
        int x = gen.nextInt(n / 2),
                y = x + 1 + gen.nextInt(n / 5),
                z = y + 1 + gen.nextInt(n / 5);
        for (int k = 0; k < n; k++)
            if (k != x && k != y && k != z)
                bst.remove(k);
        ...
        assertNotNull(loc);
        assertFalse(loc.dirty);
        assertNull(bst.search(x).getBefore());
        assertNull(bst.search(z).getAfter());
        assertTrue(y == bst.search(x).getAfter().data);
        assertTrue(z == bst.search(y).getAfter().data);
        assertTrue(x == bst.search(y).getBefore().data);
        assertTrue(y == bst.search(z).getBefore().data);
    }
    ...
    public void beforeAVL() {
        BinarySearchTree<Integer> avl =
                new AVLTree<>((Integer x, Integer y) -> x < y);
        assertTrue(avl.isEmpty());
        int[] a = new int[]{
                12, 4, 18, 5, 11, 8, 15, 9, 17, 20, 3,
                13, 19, 2, 14, 7, 6, 10, 1, 16
    ...
    public void mediumAVLtest() {
        int[] a;
        AVLTree<Integer> avl = new AVLTree<>((x, y) -> x < y);
        a = new int[]{
                5, 3, 1, 2, 7, 6, 9, 8, -1, -7, -5, -2, -3,
                10, 15, 13, 12, 19, 20, 21
        };
        for (Integer x : a) {
            avl.insert(x);
        }
    ...
    public void keysDirtyAVL() {
        AVLTree<Integer> avl = new AVLTree<>((x, y) -> x < y);
        int[] a = new int[]{
                26, 5, 28, 32, 4, 8, 0, 2, 3, 6,
                10, 12, 22, 1, 14, 20, 16, 18, 7, 24, 30
        };
        for (Integer key : a)
            avl.insert(key);
        List<Integer> ks = avl.keys();
    ...
    private <K> int countDirtyNodes(BinarySearchTree<K>.Node p) {
        if (p == null)
            return 0;
        return (p.dirty ? 1 : 0) +
                countDirtyNodes(p.left) + countDirtyNodes(p.right);
    }
    ...
    private <K> boolean verifyParentPointersHelper(BinarySearchTree<K>.Node p,
                                                   BinarySearchTree<K>.Node q) {
        if (q == null)
            return true;
        if (q.parent != p)
            return false;
        return verifyParentPointersHelper(q, q.left) &&
                verifyParentPointersHelper(q, q.right);
    }

    private <K>
    boolean verifyOrderingProperty(BinarySearchTree<K>.Node p,
    ...
    private <K>
    boolean allGreaterThan(BinarySearchTree<K>.Node p, K x,
                           BiPredicate<K, K> lessThan) {
        if (p == null) return true;
        return !lessThan.test(p.data, x) &&
                allGreaterThan(p.left, x, lessThan) &&
                allGreaterThan(p.right, x, lessThan);
    }

    private <K>
    boolean allLessThan(BinarySearchTree<K>.Node p, K x,
                        BiPredicate<K, K> lessThan) {
        if (p == null) return true;
        return lessThan.test(p.data, x) &&
                allLessThan(p.left, x, lessThan) &&
                allLessThan(p.right, x, lessThan);
    }
